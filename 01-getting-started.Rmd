...
假如我们打算从 `photos` 目录中找出今年三月拍摄的照片，并将其文件名称导出到 `mar_photos.txt` 这个文本文件中，那么在命令行中可以执行以下命令：

```bash
xiaodong@codeland:~$ cd photos; ls -l | grep 'Mar' | awk '{ print $9 }' > mar_photos.txt
```
